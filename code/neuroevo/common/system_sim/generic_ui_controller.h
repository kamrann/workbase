// generic_ui_controller.h

#ifndef __NE_GENERIC_UI_CONRTOLLER_H
#define __NE_GENERIC_UI_CONRTOLLER_H
...
namespace sys
{
	...
	class generic_ui_controller_defn:
	...
	public:
		...
		generic_ui_controller_defn(num_eff_provider_fn_t num_eff_prov_fn);
...
//		static std::unique_ptr< generic_ui_controller > generate(ddl::navigator nav, size_t num_effectors);
	...
	public:
		generic_ui_controller(size_t num_effectors);

	protected:
		double get_current_modulation(input::input_stream_access const& input_data) const;
		virtual double get_activation(unsigned long effector, input::input_stream_access const& input_data) const override;

	protected:
		std::map< input::btn_element_id, double > modulations_;

		friend class generic_ui_controller_defn;
	};
