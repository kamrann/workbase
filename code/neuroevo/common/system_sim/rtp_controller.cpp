...
namespace sys
{
	bimap< i_controller::Type, std::string > const i_controller::TypeMp = {
		//		{ "Evolved", sys::i_controller::Type::Evolved },
			{ sys::i_controller::Type::Passive, "Passive" },
			{ sys::i_controller::Type::Interactive, "Interactive" },
			//		{ "Database", sys::i_controller::Type::Database },
			{ sys::i_controller::Type::Preset, "Preset" },
	};

	i_controller::Type i_controller::decode_type(std::string const& as_str)
	{
		return TypeMp.right.at(as_str);
	}


#if 0
	prm::param i_controller::get_controller_agent_type(prm::param_accessor param)
	{
		...
		prm::param_accessor param_vals)
	{
		...
		for(auto evolved_entry : node)
		{
			...
			auto controller_agent_type = compatible_node[0].as< std::string >();
			if(controller_agent_type != agent_type.as< std::string >())
			{
				continue;
			}
	...
	std::string i_controller::update_schema_providor(prm::schema::schema_provider_map_handle provider, prm::qualified_path const& prefix)
	{
		...
		(*provider)[path + std::string("controller_type")] = [](prm::param_accessor)
		{
			std::vector< std::pair< std::string, std::string > > enum_elements;

			typedef decltype(TypeMp.left) mp_t;
			std::transform(
				std::begin(TypeMp.left),
				std::end(TypeMp.left),
				std::back_inserter(enum_elements),
				[](mp_t::value_type const& entry)
			{
				return std::make_pair(std::to_string((int)entry.first), entry.second);
			});
		...
		(*provider)[path + std::string("evolved_controller_sel")] = [=](prm::param_accessor param_vals)
		{
			...
			auto controllers = get_compatible_evolvable_controllers(agent_type_node, param_vals);
...
// TODO:		auto spaceship_bindings_rel = spaceship_interactive_controller::update_schema_provider(provider, path);

		(*provider)[path] = [=](prm::param_accessor param_vals)
		{
			auto s = sb::list("controller_type_specific_params");

			if(param_vals.is_available("controller_type"))
			{
				auto epv = prm::extract_as< prm::enum_param_val >(param_vals["controller_type"]);
				auto type = decode_type(epv[0]);
				switch(type)
				{
#if 0
					case Evolved:
	...
	std::unique_ptr< i_controller_factory > i_controller::create_factory(prm::param_accessor param)
	{
		return{};
#if 0
