...
//			std::mutex cmd_mx;
		...
		struct innermost_state_base: public sc::state < MostDerived, Context >
		{
			...
			sc::result react(ev_update_prompt const&)
			{
				...
				context< system_controller >().prompt_callback(prompt);
		...
		struct not_ready:
			...
			innermost_state_base< not_ready, inactive >,
			i_cmd_state
		{
			using innermost_state_base::innermost_state_base;
			using innermost_state_base::react;
		...
		struct ready:
			...
			innermost_state_base< ready, inactive >,
			i_cmd_state
		{
			using innermost_state_base::innermost_state_base;
			using innermost_state_base::react;
...
//			sc::simple_state < paused, not_running >,
			innermost_state_base< paused, not_running >,
			i_cmd_state
		{
			using innermost_state_base::innermost_state_base;
			using innermost_state_base::react;
		...
		struct completed:
			innermost_state_base< completed, not_running >,
			i_cmd_state
		{
			using innermost_state_base::innermost_state_base;
			using innermost_state_base::react;
		...
		struct running:
			innermost_state_base< running, active >,
			i_cmd_state
		{
			using innermost_state_base::innermost_state_base;
			using innermost_state_base::react;
