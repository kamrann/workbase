...
namespace rtp_sat {
	...
	{
		return AngularFullStop;
	}

	template < WorldDimensionality dim >
	i_param_widget* sat_scenario< dim >::enum_param_type::create_widget() const
	{
...
*/		case FullStop:
			return new full_stop< dim >(param_pr.second);
		case TargetOrientation:
			return new target_orientation< dim >(param_pr.second);
		default:
			return nullptr;
		}
	}
	...
	template < WorldDimensionality dim >
	typename sat_system< dim >::state angular_full_stop< dim >::generate_initial_state(rgen_t& rgen)
	{
