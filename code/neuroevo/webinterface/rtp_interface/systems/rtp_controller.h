...
#ifndef __NE_RTP_CONTROLLER_H
#define __NE_RTP_CONTROLLER_H
...
namespace YAML {
	...
	template <>
	struct convert< rtp::i_controller::Type >
	{
		static Node encode(rtp::i_controller::Type const& rhs)
		{
			return Node(rtp::i_controller::Names[rhs]);
		}

		static bool decode(Node const& node, rtp::i_controller::Type& rhs)
		{
			if(!node.IsScalar())
			{
				return false;
		...
		static std::map< std::string, rtp::i_controller::Type > const mapping_;
	};
}
